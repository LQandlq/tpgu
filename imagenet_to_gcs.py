...
def _convert_to_example(filename: str, image_buffer: str, label: int, synset: str, height: int,
...
def _is_cmyk(filename: str) -> bool:
    ...
    # File list from:
    # https://github.com/cytsai/ilsvrc-cmyk-image-list
    denylist = set(['n01739381_1309.JPEG', 'n02077923_14822.JPEG', 'n02447366_23489.JPEG', 'n02492035_15739.JPEG',
...
def _process_image_files_batch(coder: ImageCoder, output_file: str, filenames: Iterable[str],
                               synsets: Iterable[Union[str, bytes]], labels: Mapping[str, int]):
...
def _process_dataset(filenames: Iterable[str], synsets: Iterable[str], labels: Mapping[str, int], output_directory: str,
                     prefix: str, num_shards: int) -> List[str]:
    ...
    files = []
    # add a readme file to the output directory, so that we know which files are in which shard
    for shard in range(num_shards):
        ...
        with open(PurePath(output_directory, f'{PurePath(output_file).name}_internal_files.txt'), 'a',
                  encoding='utf-8', errors='ignore') as metadata:
            metadata.write(f'shard {PurePath(output_file).name} contains the following files:\n')
            metadata.writelines([f'{PurePath(file).name}\n' for file in chunk_files])
    return files
...
def convert_to_tf_records(raw_data_dir: str, local_scratch_dir: str) -> Tuple[List[str], List[str]]:
    ...
    logging.info('Globing all the validation files...')
    ...
    logging.info('Getting validation file synset labels from the directory name')
    ...
    logging.info('Globing all the training files')
    training_files = tf.gfile.Glob(os.path.join(raw_data_dir, TRAINING_DIRECTORY, '*', '*.JPEG'))

    logging.info('Getting training file synset labels from the directory name')
    ...
    logging.info('Creating unique ids for all synsets')
    ...
    training_records = _process_dataset(
        ...
        labels=labels,
        output_directory=os.path.join(local_scratch_dir, TRAINING_DIRECTORY),
        prefix=TRAINING_DIRECTORY,
    ...
    #     labels=labels,
    #     output_directory=os.path.join(local_scratch_dir, VALIDATION_DIRECTORY),
    #     prefix=VALIDATION_DIRECTORY,
    #     num_shards=VALIDATION_SHARDS)

    return training_records, validation_records


def upload_to_gcs(training_records: Iterable[str], validation_records: Iterable[str], gcs_output_path: str,
                  gcs_project: str, client: storage.Client = None):
...
def run(raw_data_dir: str, gcs_upload: bool, gcs_project: str, gcs_output_path: str, local_scratch_dir: str,
    ...
    if raw_data_dir is None:
        raise AssertionError('The ImageNet download path is no longer supported. Please download '
                             'and extract the .tar files manually and provide the `raw_data_dir`.')

    # Convert the raw data into tf-records
    training_records, validation_records = convert_to_tf_records(raw_data_dir=raw_data_dir,
                                                                 local_scratch_dir=local_scratch_dir)

    # Upload to GCS
    if gcs_upload:
        upload_to_gcs(training_records=training_records, validation_records=validation_records,
                      gcs_output_path=gcs_output_path, gcs_project=gcs_project, client=client)


def main(_):
    run(raw_data_dir=FLAGS.raw_data_dir, gcs_upload=FLAGS.gcs_upload, gcs_project=FLAGS.project,
        gcs_output_path=FLAGS.gcs_output_path, local_scratch_dir=FLAGS.local_scratch_dir)
