# This file is based on https://github.com/rishigami/Swin-Transformer-TF/blob/main/swintransformer/model.py
import math
...
class TaylorPolynomial(tf.keras.layers.Layer):
    def __init__(self, order, dim):
        ...
        self.a = [self.add_weight(shape=[dim],
                                  initializer=dict(class_name='TruncatedNormal',
                                                   config=dict(stddev=.02)) if i != 0 else Initializers.Constant(1.0),
                                  trainable=True,
...
class Mlp(tf.keras.layers.Layer):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0., prefix=''):
        ...
        self.polynomial = TaylorPolynomial(3, hidden_features)
        self.fc2 = Dense(out_features, name=f'{prefix}/mlp/fc2')
        self.drop = Dropout(drop)

    def call(self, x):
        x = self.fc1(x)
        x = self.polynomial(x) * x
        x = self.drop(x)
        x = self.fc2(x)
        x = self.drop(x)
        return x
